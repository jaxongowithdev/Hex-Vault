...
class _MoveItemViewState extends State<MoveItemView> {
  ...
  Future<void> _moveItem() async {
    if (_selectedContainer == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Pick a destination chest')));
      return;
    }
    ...
    if (destCount >= _selectedContainer!.capacity) {
      final confirm = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          title: const Text('That chest is full'),
          ...
          : ListView(
              ...
              children: [
                Card(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('RESEATING', style: GoogleFonts.sourceSans3(color: VisualTheme.secondaryColor, letterSpacing: 1.4, fontSize: 11, fontWeight: FontWeight.w700)),
                        const SizedBox(height: 6),
                        Text(widget.item.name, style: GoogleFonts.cinzel(fontSize: 22, fontWeight: FontWeight.w700)),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(color: VisualTheme.primaryColor, borderRadius: BorderRadius.circular(14)),
                  child: Text('Now in ${_currentContainer?.name ?? 'unknown'}${_currentContainer != null ? ' / ${_currentContainer!.room}' : ''}', style: const TextStyle(color: Color(0xFFF6EFE3))),
                ),
                const SizedBox(height: 20),
                Text('Move into', style: GoogleFonts.cinzel(fontSize: 20, fontWeight: FontWeight.w700)),
                const SizedBox(height: 10),
                if (_containers == null || _containers!.isEmpty)
                  const Card(child: Padding(padding: EdgeInsets.all(28), child: Center(child: Text('No other chests yet'))))
                else
                  ..._containers!.map((c) {
                ...
                FilledButton.icon(key: const ValueKey('confirm_move_button'), onPressed: _selectedContainer == null ? null : _moveItem, icon: const Icon(Icons.swap_horiz), label: const Text('Reseat piece')),
              ],
            ),
    );
  }
